...
export default function ServicesSection() {
  ...
  const handleScroll = debounce(() => {
    if (carouselRef.current) {
      ...
      if (newIndex !== activeIndex && newIndex >= 0 && newIndex < commonIssues.length) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && isMobile && carouselRef.current) {
      ...
      setTimeout(() => {
        const cardElements = carouselRef.current?.querySelectorAll('.issue-card');
      ...
      <div className="container mx-auto px-3 xs:px-4 max-w-7xl">
        <div className="text-center mb-8 xs:mb-10 sm:mb-16">
          ...
          <p className="text-[#2b3343] max-w-2xl mx-auto text-base sm:text-lg leading-relaxed">
            Nos techniciens sont formés pour diagnostiquer et réparer rapidement tous types de pannes
          </p>
        </div>
            ...
            <div className="overflow-hidden px-4">
              ...
              >
                {commonIssues.map((issue, index) => (
                  <div 
                    key={index} 
                    className="flex-shrink-0 snap-center issue-card w-full transition-transform duration-300"
